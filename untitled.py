...
class Ui_MainWindow(QObject):
    update_ui_signal = pyqtSignal(str, str)  # Signal for updating the UI from another thread

    def __init__(self):
        super().__init__()
        self.update_ui_signal.connect(self.show_message)  # Connect the signal to the slot
    ...
    def check_time_against_database(self):
        while True:
            ...
            for time_str in formatted_times:
                scheduled_time_obj = datetime.strptime(time_str, '%H:%M')
                time_difference = scheduled_time_obj - current_time_obj

                if time_difference == timedelta(minutes=5):
                    self.update_ui_signal.emit("Reminder", "Five minutes remaining until sleep")
